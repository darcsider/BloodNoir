...
class Actor
{
	protected:
		...
		World*					m_currentWorld;			// World that the actor is currently set on
	...
	public:
		...
		World* GetActorCurrentWorld() { return m_currentWorld; }
		// set the world that the Actor is attached to currently
		void SetActorCurrentWorld(World *world) { m_currentWorld = world; }
		...
		void BuildActor(string fileName, World* world, Sprite* sprite);
